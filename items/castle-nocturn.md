---
title: Castle Nocturn Loot
description: Every custom item from Dungeon 2 — Sanguine gear, Bloodfruit, Nocturn Tomes, and vampire relics.
published: true
date: 2026-07-05T00:00:00.000Z
tags: items, castle-nocturn, dungeon-2
editor: markdown
dateCreated: 2026-07-05T00:00:00.000Z
---

# Castle Nocturn Loot

Custom items from [Dungeon 2](/dungeons/castle-nocturn). Castle Nocturn's loot is a full step above the Viking Stronghold: diamonds and emeralds off trash mobs, rare netherite, Tier-II tomes, and the Sanguine crafting line.

## Sanguine Materials

The castle's vampires carry the materials for blood-forged gear.

| | Item | Lore | Source |
| --- | --- | --- | --- |
| <img src="/assets/items/materials/vampire_dust.png" alt="" width="32"> | **Vampire Dust** | The remains of the sanguine undead | Guaranteed from every vampire mob |
| <img src="/assets/items/materials/sanguine_ore.png" alt="" width="32"> | **Sanguine Ore** | Blood-veined ore | 5% from [Vampire Laborers](/monsters/castle-nocturn/vampire-laborer) |
| <img src="/assets/items/materials/sanguine_ingot.png" alt="" width="32"> | **Sanguine Ingot** | Smelted sanguine metal | 1% from Vampire Laborers |

## Sanguine Gear

| | Item | What It Does |
| --- | --- | --- |
| <img src="/assets/items/weapons/sanguine_sword.png" alt="" width="32"> | **Sanguine Sword** | +9 attack damage. Passive **Bloodthirst**: heals you for 1% of damage dealt. |
| | **Sanguine Chestplate** | +1 heart. Part of the Sanguine Set. |
| | **Sanguine Platelegs** | +1 heart. Part of the Sanguine Set. |
| | **Sanguine Boots** | +1 heart. Part of the Sanguine Set. |

**Sanguine Set Bonus** — wearing the set pays off:

| Pieces Worn | Bonus |
| --- | --- |
| 3 pieces | +5 total hearts |
| Full set | +2 extra hearts on top |

The chestplate also carries a **gem socket** — see [Gear Gems](/items#gear-gems).

## Bloodfruit

| | Item | What It Does | Source |
| --- | --- | --- | --- |
| <img src="/assets/items/castle-nocturn/bloodfruit_seed.png" alt="" width="32"> | **Bloodfruit Seed** | Plant it and let it grow through four stages. | 3% from vampire mobs |
| <img src="/assets/items/castle-nocturn/blood_fruit.png" alt="" width="32"> | **Blood Fruit** | Consume for **Vampyric Blessing**: 5% lifesteal for 120s, and while active a 5% chance to cheat death, reviving at 4 hearts. | Grown from seeds; 5% direct from bosses |

## Relics

| | Item | What It Does | Source |
| --- | --- | --- | --- |
| <img src="/assets/items/weapons/final_oath.png" alt="" width="32"> | **The Final Oath** | Right-click **Broken Oath**: summons a spectral blade at the block you're looking at — 15 damage to mobs within 2 blocks. 30s cooldown. | 2% from the [Oathbreaker](/monsters/castle-nocturn/oathbreaker) |
| <img src="/assets/items/weapons/holy_hand_grenade.png" alt="" width="32"> | **Holy Hand Grenade** | Right-click: throws a holy grenade. After 3 seconds it explodes — 10 damage to mobs, 10 healing to players, and removes curses. 10-block range. | Blessed relic — crafted/special reward |
| | **Vampyric Repair Kit** | Drag onto damaged items to restore 50 durability. | 1–2 guaranteed from [Vraelith](/monsters/castle-nocturn/vraelith) |

## Nocturn Tomes

Tier-II enchant books. ~5–6% from regular mobs, guaranteed from bosses (plus a 50% second roll).

| Tome | Enchant |
| --- | --- |
| Nocturn Tome — Protection II | Protection II |
| Nocturn Tome — Sharpness II | Sharpness II |
| Nocturn Tome — Power II | Power II |
| Nocturn Tome — Efficiency II | Efficiency II |
| Nocturn Tome — Unbreaking II | Unbreaking II |
| Nocturn Tome — Feather Falling II | Feather Falling II |
| Nocturn Tome — Looting I | Looting I |

**Beheading I books** also drop here — [Blood Acolytes](/monsters/castle-nocturn/blood-acolyte) are by far the best source at 20%, and bosses have a 50% chance.

## Treasure

Trash mobs drop straight treasure: diamonds and emeralds at 4–22% depending on the mob, and netherite ingots from 0.1% (trash) up to 12% (bosses). Bosses guarantee 8–14 diamonds and 8–14 emeralds per kill.

## Related

- [Jar of Holy Water](/items/potions) — the curse cure
- [Souls & Currency](/items/souls) — Sanguine Souls come from these bosses
- [Viking Stronghold Loot](/items/viking-stronghold)
